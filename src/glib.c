...
{	Edge edge1,edge2;
	...
	edge1->prevedge = EMPTY;
	edge1->nextedge = g[n].adj_list;
	if (edge1->nextedge != EMPTY)
	...
	edge2->prevedge = EMPTY;
	edge2->nextedge = g[m].adj_list;
	if (edge2->nextedge != EMPTY)
...
{	Edge edge;
	...
	while (edge!=EMPTY && edge->endpoint!=j)
		edge = edge->nextedge;
...
{	Edge other;
	...
	if (edge->prevedge == EMPTY) {
		graph[i].adj_list = edge->nextedge;
		if (edge->nextedge != EMPTY)
			edge->nextedge->prevedge = EMPTY;
		}
	else if (edge->nextedge == EMPTY)
        	(edge->prevedge)->nextedge = EMPTY;
	else {
		(edge->nextedge)->prevedge = edge->prevedge;
		(edge->prevedge)->nextedge = edge->nextedge;
		}
	if (other->prevedge == EMPTY) {
		graph[j].adj_list = other->nextedge;
		if (other->nextedge != EMPTY)
			other->nextedge->prevedge = EMPTY;
		}
	else if (other->nextedge == EMPTY)
		(other->prevedge)->nextedge = EMPTY;
	else {
